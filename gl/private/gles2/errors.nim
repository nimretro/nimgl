...
proc glGetError*(): GLenum {.stdcall, importc, dynlib: dllname}
...
macro wrapErrorChecking*(f: untyped): typed =
  ...
  for child in f.children:
    ...
    glProc.pragma = newTree(nnkPragma,
        newTree(nnkExprColonExpr, ident"importc" , newLit($child.name)),
        newTree(nnkExprColonExpr, ident"dynlib" , newLit($dllname)))
